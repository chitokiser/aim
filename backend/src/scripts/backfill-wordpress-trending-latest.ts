 ...
 * (실시간 이슈) articles to the WordPress trending site, newest first. Unlike
 * BlogService.listWordPressCandidates (oldest-first, used by the daily retry
 * cron), this picks the latest content regardless of how old the backlog is —
 * for catching up a freshly-connected WordPress site with what's current right
 * now. Safe to re-run: backfillWordPressPost skips anything already posted.
 *
 * Run: npx ts-node -r dotenv/config src/scripts/backfill-wordpress-trending-latest.ts
 */
...
const LIMIT = 20;
// Same "substantial, not thin" gate as WordPressSchedulerService's candidates.
...
async function main() {
  ...
  try {
    ...
    if (!wordpress.isConfigured('trending')) {
      console.error('WordPress "trending" target is not configured (missing WORDPRESS_TRENDING_* vars in .env).');
      process.exitCode = 1;
      return;
    }
    ...
    for (const post of posts) {
      ...
      if (result.status === 'posted') {
        ...
        console.log(`[skip] ${post.title} (category no longer maps to the trending WordPress target)`);
      } else {
